...
attainment_data <- data.frame(
  ...
  idaci = rnorm(100, mean = 0, sd = 15),
  ses = rpois(n = 100, lambda = 0),
  home_to_school_travel_time = rpois(n = 100, lambda = 50),
  age = rpois(n = 100, lambda = 0)
)
...
# Create a class "neural_network_model" defined by "x", the predictors, "y", the outcomes, and "n_layers", the number of layers
...
# Create a second object of class "generalised_linear_model" using the generator function
ses_generalised_linear_model <- generalised_linear_model(
  y = "ses",
  x = list("home_to_school_travel_time"),
  family = function(){poisson(link = "log")}
)

# Create an object of class "neural_network_nodel" using the generator function
...
- Specifically, a function that is generalised to different types of inputs to its arguments so that the specific methods being called depends on the type of inputs to its arguments 
...
# Create a generic "model_description" with the output define to be a character string
setGeneric(
...
# Create a generic for fitting any types of models 
setGeneric(
  name = "fit",
...
setGeneric(
  name = "summary",
  def = function(model = NULL){
    standardGeneric(f = "summary")
  }
)
...
setClass(Class = "general_linear_model_fit")
setClass(Class = "generalised_linear_model_fit")
setClass(Class = "neural_network_model_fit")
...
setMethod(
  ...
  definition = function(model, data){
    ...
    general_linear_model_fit <- lm(formula = formula, data = data)
    class(general_linear_model_fit) <- "general_linear_model_fit"
    return(general_linear_model_fit)
  }
)
setMethod(
  f = "fit",
  ...
  definition = function(model, data){
    ...
    generalised_linear_model_fit <- glm(formula = formula, data = data, family = "poisson")
    class(generalised_linear_model_fit) <- "generalised_linear_model_fit"
    return(generalised_linear_model_fit)
  }
)
...
setMethod(
  ...
  definition = function(model, data){
    ...
    neural_network_model_fit <- neuralnet::neuralnet(formula = formula, data = data)
    class(neural_network_model_fit) <- "neural_network_model_fit"
    return(neural_network_model_fit)
  }
)

ses_generalised_linear_model_fit <- fit(science_neural_network_model, attainment_data)


neuralfit <- neuralnet(
  formula = ks2_maths ~ school_teaching_quality + idaci + ses,
  data = attainment_data,
  hidden = 3
)
e?neuralnet
neuralfit |> coefficients()

plot(neuralfit)

setClassUnion("GLM", members = c("general_linear_model", "generalised_linear_model"))
rm(summary)
