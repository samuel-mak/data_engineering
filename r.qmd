...
science_neural_network_model <- neural_network_model(
  y = "ks2_science",
  x = c("age", "ses", "idaci", "home_to_school_travel_time")
)
...
# Modify the value of the slot "family" in the instance "maths_generalised_linear_model" using the "@" operator
...
**Generic functions (Generics) and Methods**
- **Introduction**
  - A generic is a function that is able to take different input (in terms of classes of the inputs to the arguments and/or the set of arguments) and operate differently according to the input by dispatching (matching and passing the inputs to the arguments in the generic) to a specific method, a function for the specific case, yet return a conceptually similar output
- **Creating a generic**
...
- **Example**
```{r}
# The goal is to create a generic "model_description" that would take in one or two models and summaries them in a meaningful way. The way the model(s) is/are summarised depends on the type of model (a General Linear Model vs Generalised Linear Model) and the number of models fed into this generic. 

# Create a generic "model_description" with the output defined to be a character string
# The goal of this generic is to take in one or two models and to summarise/compare them
setGeneric(
  name = "model_description",
  def = function(model_1 = NULL, model_2 = NULL){
    standardGeneric(f = "model_description")
  },
  valueClass = "character"
)

# Create a generic "fit" for fitting any types of models 
# The goal of this generic is to fit the specified model to data
setGeneric(
  name = "fit",
...
# The goal of this is to use this generic summary to summarise the models fitted by the fit generic by adding specific S4 methods to it 
...
- **Tips**
  ...
  - All generics and their signatures for objects of one or more classes can be assessed by the `showMethods` function (`showMethods(classes = "classname")`
- **Method and super and subclasses**
- **Example**
```{r}
# Create a method for the case when both objects are of the class "general_linear_model"
setMethod(
  f = "model_description",
...
# Create a method for the case when both objects are of the class "generalised_linear_model"
setMethod(
  f = "model_description",
...
model_description(maths_generalised_linear_model, reading_generalised_linear_model)

showMethods(f = model_description)

showMethods(class = "general_linear_model")
maths_generalised_linear_model@family()$family
...
setMethod(
  ...
  definition = function(model, data){
    ...
    neural_network_model_fit_object <- neuralnet::neuralnet(formula = formula, data = data)
...
showMethods("summary")
isS4(science_neural_network_model)

removeGeneric("summary")
removeMethod("summary", signature = signature(object = "generalised_linear_model_fit"))
removeMethod("summary", signature = signature(object = "general_linear_model_fit"))
showMethods("summary")

summary(maths_general_linear_model_fit)
summary(ses_generalised_linear_model_fit)

# Chat gpt was right 
# This works at one level only 

summary(science_neural_network_model_fit)
rm(summary)
# Call the method designed for the superclass
print(obj)
```

```{r}
science_neural_network_model_fit$weights
```



- **Coerce a non-S4 generic as an S4 generic**
...
setGeneric("some_function",
           function(object, ...) {
             standardGeneric("some_function")
           }
           )

setMethod("some_function",
          signature(object = "character"),
          function(object, character_argument) {
            # Your customized summary method here
            
            print(paste0("This is my character: ",character_argument))
          })
setMethod("some_function",
          signature(object = "numeric"),
          function(object, number_argument) {
            # Your customized summary method here
            
            print(paste0("This is my number: ", number_argument))
          })

# Now let's add an argument to the generic
setMethod("some_function",
          signature(object = "ANY", digits = "numeric"),
          function(object, digits = 2, ...) {
            # Your customized summary method here
            print(paste("Customized summary method with digits =", digits))
          })

summary

some_function(object = 1, number_argument = "this is my message")
rm(summary)


# Arguments depend on classes to other arguments (just like in DBI drv argument)
summary

model

fit_summary generic
If general then have  
```
```{r}
# methods for superclass works for all subclass 
setClass("SuperClass",
         representation(a = "numeric"))

# Define a method for the superclass
setMethod("print",
          signature(x = "SuperClass"),
          function(x) {
            cat("SuperClass object with a =", x@a, "\n")
          })
