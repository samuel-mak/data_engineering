...
reading_generalised_linear_model <- generalised_linear_model(
  y = "ks2_reading",
  x = list("ks1_maths", "school_teaching_quality"),
  family = function(){binomial(link = "logit")}
)
...
# Coerce an the file.choose function (an existing function) as an S4 generic
setGeneric(
  name = "file.choose"
)
```
  ...
  - The `setMethod` function has the general form
    - `setMethod(f, definition)`
      - ***Where***
        ...
        - `signature` is for specifying the case for the method (the classes of the value of the inputs to the arguments to the generic) through the `signature` function
        - `definition` is the method for the generic as a function
- **Tips**
  - All methods of a generic can be assessed by the `showMethod` function (`showMethod(f = generic)`)
- **Example**
```{r}
