...
- **Introduction**
  - ODBC-compliant DBMSs are worked with from R through an ODBC interface of R in the form of an R package and, as mentioned, there are two main packages, namely `DBI` and `RODBC`, that are independently capable of some basic operations. These basic operations are described with methods for both packages. 
- **Connect R to an ODBC-compliant DBMS**
  - **DBI**
    - The `dbConnect` function is a generic for connecting R to various DBMSs 
    - The `dbConnect` function has the general form and usage
      ```{r, eval = F, include = T}
      connection_object_dbi <- DBI::dbConnect(drv, ...)
      ```
        - ***Where***
          - `drv` is a function that specifies the type of the DBMS to be connected to and hence the method being called to connect R to that DBMS. For ODBC-compliant DBMSs, it would be the `odbc::odbc()` function.
          - `...` is a set of arguments that depends on the input to the `drv` argument, hence, refer to the drv function for specifics, these usually relate to the DSN 
  - **RODBC**
    - The `odbcDriverConnect` function connects R to ODBC-compliant DBMSs
    - The `odbcDriverConnect` function has the general form and usage
      ```{r, eval = F, include = T}
      connection_object_rodbc <- RODBC::odbcDriverConnect(connection)
      ```
        - ***Where***   
          - `connection` is the DSN (or connection string), usually in the form of "Driver={ODBC Driver 17 for SQL Server};Server={Server Name};Database={Database Name};"
- **
